...
orange_img = cv2.imread('./images/orange.jpg')
orange_img = cv2.resize(orange_img, dsize=(512,512))

detector = dlib.get_frontal_face_detector()
predictor = dlib.shape_predictor('./models/shape_predictor_68_face_landmarks.dat')

cap = cv2.VideoCapture(0) # 0 : webcam, './images/face.mp4'

while cap.isOpened():
    ret, img = cap.read()

    if not ret: # frame이 없으면 break
        break

    faces = detector(img) # 얼굴 영역 detect
    result = orange_img.copy()

    if len(faces) > 0:
        face = faces[0] # 0번째 얼굴만 사용
        x1, y1, x2, y2 = face.left(), face.top(), face.right(), face.bottom()
        face_img = img[y1:y2, x1:x2].copy()

        shape = predictor(img,face) # landmark 68점(0~67) 구하기
        ...
        le_x2 = shape[39, 0]
        le_y2 = shape[41, 1]
        le_margin = int((le_x2 - le_x1) * 0.18) # 너무 tight하지 않게 margin
        ...
        result = cv2.seamlessClone( # 티나지 않게 합성
            right_eye_img,
            result,
